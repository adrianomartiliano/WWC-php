...
if (!isset($_SESSION['loggedin']) || $_SESSION['admmaster'] != 'S') {
...
try {
    ...
    $stmt = $conn->prepare("UPDATE rankingx1prata SET posicao = 0 WHERE posicao = ?");
    $stmt->bind_param("i", $nova_posicao);
    $stmt->execute();

    // Atualiza a posição do jogador na posição atual
    $stmt = $conn->prepare("UPDATE rankingx1prata SET posicao = ? WHERE posicao = ?");
    $stmt->bind_param("ii", $nova_posicao, $posicao);
    $stmt->execute();

    // Atualiza a posição temporária do jogador na nova posição para a posição atual
    $stmt = $conn->prepare("UPDATE rankingx1prata SET posicao = ? WHERE posicao = 0");
    ...
    header("Location: ../ranking_x1_prata.php");
    exit();
} catch (Exception $e) {
